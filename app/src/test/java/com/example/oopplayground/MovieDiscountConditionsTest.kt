...
import com.example.oopplayground.model.discounts.policy.AmountDiscountPolicy
...
import com.example.oopplayground.model.discounts.policy.ZeroDiscountPolicy
...
class MovieDiscountConditionsTest {
    ...
    @Test
    fun `Movie fee not discounted with not matching any conditions`() {
        val policy = AmountDiscountPolicy(
            discountAmount = Money.wons(800),
            SequenceCondition(1),
            SequenceCondition(10),
    ...
    @Test
    fun `Movie fee not discounted with not matching PeriodConditions`() {
        val policy = AmountDiscountPolicy(
            discountAmount = Money.wons(800),
            PeriodCondition(
                DayOfWeek.MONDAY,
    ...
    @Test
    fun `Movie fee not discounted with ZeroDiscountPolicy`() {
        val policy = ZeroDiscountPolicy()
        ...
        assert(screening.calculateFee(1).amount.toInt() == 10000)
    }
}
